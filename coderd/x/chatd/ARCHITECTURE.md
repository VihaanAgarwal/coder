...
1. The endpoint applies the `RequestCompaction` transition: only allowed from `W`, sets `chats.compaction_requested_at = now()` plus `chats.compaction_requested_by_api_key_id` (the caller's key), lands in `R0` without inserting any message, and publishes a status-change pubsub event to wake workers. A timestamp is used instead of a boolean for debuggability. Because the compaction turn inserts no user message, the worker's model resolution falls back to the recorded request key for AI Gateway attribution when the prior history carries no active-turn key.
